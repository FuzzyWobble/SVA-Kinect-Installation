...
//------------------------------------------------------------
void particle::draw(float sz){
	ofSetCircleResolution((sz/2.0)+3);
	ofCircle(pos.x, pos.y, sz);
	//ofTriangle(pos.x,pos.y,pos.x+(sz/2.0),pos.y+sz,pos.x+sz,pos.y);
}
...
//------------------------------------------------------------
void particle::trailUpdate(){
	
	ofPoint pt;
	pt.set(pos.x,pos.y);
	trail.push_back(pt);
	
	if(trail.size()>trailSize){ //ensure trail doesn't get longer than trailSize
		trail.erase(trail.begin()); //erase first
	}
}
...
//------------------------------------------------------------
bool particle::addRepulsionForce(particle &p, float radius, float scale){
...
void particle::xenoToPoint(float spd){
	...
	pos.x = spd * posOrigin.x + (1-spd) * pos.x; 
	pos.y = spd * posOrigin.y + (1-spd) * pos.y;
